...
void printMenu() // This method gives the options that can be selected by our user
{
    std::cout << "\n\nSelect from the following menu:\n"
            ...
            << "10) Insert value into second List\n"
            << "11) Merge Lists\n"
            << "12) Print List\n"
            << "13) Remove All Duplicates\n"
            << "14) Remove Paris\n"
            << "Your choice: ";
}
...
int main() {
    ...
    DoubleLinkedList<int>* merged = nullptr;
    ...
    while (answer!=7) { // This loop runs until the user answers 7 to exit
        ...
        if (answer ==1) { // If the answer is one (push to front), we get the user to input a value to add. Then, we add the answer to the front of the DoubleLinkedList
           ...
           if(test.remove(temp)) { // If the value is found, the value is removed
         ...
         else if(answer ==8) { // If the value is seven, we let the user know that the program is exiting

             std::cout << "" <<endl;
             std::cout << "Checking if sorted"<<endl;
        ...
        else if(answer ==9) { // If the value is seven, we let the user know that the program is exiting
         ...
         else if (answer ==10) { // If the answer is one (push to front), we get the user to input a value to add. Then, we add the answer to the front of the DoubleLinkedList

            std::cout << "Give a value: "<<endl; // We ask the user to enter a value
            std::cin >> temp; // We store the value in temp
            std:: cout<<""<<endl;
            std:: cout << temp << " added to the front of the second list."<<endl;

            test1.pushFront(temp); // We use the pushFront method to add the value to the front of the DoubleLinkedList
         }

         else if (answer ==11) {

            std::cout << "Merging Lists... "<<endl; // We ask the user to enter a value
            try {
                merged = test1.mergeLists(test,test1);
                std::cout << "Merge Successful "<<endl; // We ask the user to enter a value
                ...
                std::cout << e.what()<<endl; // We ask the user to enter a value
            ...
            std::cout << "Printing Merged List "<<endl; // We ask the user to enter a value
            merged->printList();
            ...
            std::cout << "Removing Duplicates"<<endl; // We ask the user to enter a value
            ...
            int temper=0;
            std::cout << "Enter a value to remove pairs on "<<endl; // We ask the user to enter a value
            std::cin>>temper;

            if(test.removePairs(temper)) {
                std::cout<<"All pairs removed"<<std::endl;
            }
            else {
                std::cout<<"No pairs removed"<<std::endl;
            }
         }
