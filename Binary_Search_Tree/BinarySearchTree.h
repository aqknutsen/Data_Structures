...
private:
    ...
    Node<T>* m_root; // Represents a pointer that points to the root Node in a BS


    T sumEntireTree(Node<T>* subtree);

    int maxDepth(Node<T>* subtree) const;

    int countNodes(Node<T>* subtree) const;
